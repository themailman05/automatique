...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --trello-card)  TRELLO_CARD="$2"; shift 2 ;;
    --help|-h)
...
if [[ -n "$TRELLO_CARD" && -n "${TRELLO_API_KEY:-}" && -n "${TRELLO_TOKEN:-}" ]]; then
  TRELLO_IN_PROGRESS="698863532d2d3df37df5b0b6"
  curl -s -X PUT "https://api.trello.com/1/cards/${TRELLO_CARD}/idList" \
    -d "value=$TRELLO_IN_PROGRESS" \
    -d "key=$TRELLO_API_KEY" \
    -d "token=$TRELLO_TOKEN" > /dev/null 2>&1 && \
    echo "  📋 Trello card moved to In Progress" || true
fi
...
$(if [[ -n "$TRELLO_CARD" ]]; then echo "### Trello"; echo "https://trello.com/c/$TRELLO_CARD"; fi)
